...
Deno.serve(async (req) => {
  try {
    ...
    // This is a scheduled automation - no user auth needed
    // Create 1-2 new ride requests
    const numRequests = Math.floor(1 + Math.random() * 2);
    const created = [];
    ...
    console.log(`Created ${created.length} AI ride requests`);

    return Response.json({
      success: true,
      message: `Created ${created.length} AI ride requests`,
