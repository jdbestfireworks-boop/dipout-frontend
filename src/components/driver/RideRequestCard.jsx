...
import { MapPin, Navigation, Banknote, CreditCard } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { motion } from 'framer-motion';

export default function RideRequestCard({ ride, onSelect }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      onClick={() => onSelect(ride)}
      className="rounded-2xl border border-border bg-card p-5 cursor-pointer hover:border-primary/50 hover:shadow-md transition-all"
    >
      {/* Header - Fare & Payment */}
